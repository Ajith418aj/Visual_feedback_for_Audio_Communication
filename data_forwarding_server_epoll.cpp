...
// vector<string> hostClients; // class can be created for this, this is used to store the clients in a room
map<string, string> roomID_Hosts;                       // This is a map that stores all the call hosts of a particular room
map<string, vector<pair<string, int>>> roomID_IP_ports; // This is a mapping between all the IPs associated with each roomID
...
int handleTCPSocketEvent(int socket_fd, int epoll_fd)
{
    ...
    // if (getpeername(socket_fd, (struct sockaddr *)&client_address, &client_address_len) == -1)
    // {
    ...
    if (ReqType == "Create")
    {
        ...
        struct sockaddr_in udp_address1, udp_address2, udp_address3; // Do we need to introduce a third udp port for rr ?
    ...
    {
        // Control server is handling this part.
    }
    return 0;
}
void handleUDPSocketEvent(int room_socket) // room sockets include audio, ack and rr sockets
{
    ...
    while (true)
    {
        ...
        int audio_valread = recvfrom(room_socket, buffer, sizeof(buffer), 0, reinterpret_cast<struct sockaddr *>(&udp_address), &udp_address_len);
        if (audio_valread > 0)
        {
        ...
        char test_audio[20] = "audio_dummy"; // similar to audio dummy, we need to add ack dummy and reception report dummy
        char test_ack[20] = "ack_dummy";     // dummy packet for acks.
        char test_rr[20] = "rr_dummy";       // dummy rr dummy

        // To get all the clients associated with a particular roomID, we can traverse it via the map of roomIDs and the corresponding list of IP addresses.
        map<int, string>::iterator it;
        string roomid;
        it = audioSocket_RoomID.find(room_socket);
        if (it != audioSocket_RoomID.end())
        {
            roomid = audioSocket_RoomID[room_socket];
        }
        ...
        {
            roomID_IP_ports[roomid].push_back(make_pair(sender_ip, sender_port));
        ...
        // else if( it is a pure ack packet)
        // {
        ...
        else // it is an audio packet
        {
            ...
            {
                if (clientIP.first != senderIP && clientIP.second != sender_port) // now checking whether or not the IP address and the port number both are different.
                {
                    ...
                    if (inet_pton(AF_INET, clientIP.first.c_str(), &(clientAddress.sin_addr)) <= 0)
                    {
                        cerr << "Invalid client IP address" << endl;
                        return;
                    }
                    // Forward the data to the clients
                    ssize_t bytesSent = sendto(room_socket, buffer, audio_data.length(), 0,
...
int main()
{
    ...
    while (true)
    {
        ...
        for (int i = 0; i < num_events; ++i)
        {
            ...
            {
                cout<<"registerd successfully in the epoll\n";
